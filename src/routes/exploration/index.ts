  ...
  app.get('/exploration', {
    schema: { response: { 200: { type: 'object', additionalProperties: true, properties: {
      ...
      mapItems: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['id', 'title', 'countries', 'severity'], properties: {
        url: { type: ['string', 'null'] }, category: { type: 'string' }, locations: { type: 'array', items: { type: 'object', additionalProperties: true } },
    ...
    const { rows } = await app.pool.query<{
      ...
      SELECT (SELECT count(*)::int FROM filtered) AS total,
        ...
        COALESCE((SELECT jsonb_agg(jsonb_build_object(
          ...
          'url', (SELECT ri.url FROM raw_items ri WHERE ri.cyber_event_id = filtered.id
            AND ri.url ~* '^https?://' ORDER BY ri.collected_at, ri.id LIMIT 1)
