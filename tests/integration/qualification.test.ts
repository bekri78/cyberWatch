...
describe('P0 — qualification et provenance PostgreSQL', () => {
  ...
  it('P1 : relie les pays réellement extraits au flux après qualification', async () => {
    ...
    try {
      ...
      expect(response.json().mapItems).toEqual([{ id: gdeltId, title: event.title, countries: event.countries, severity: event.severity, category: event.category, locations: [], url: event.publications[0]?.url }]);
